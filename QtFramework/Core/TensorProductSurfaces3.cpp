                                         ...
                                         GLdouble& x, GLdouble& y, GLdouble& z) const
{
    ...
    const DCoordinate3 &cp = _data(row, column);
...
GLboolean TensorProductSurface3::RenderData(GLenum render_mode) const
{
    ...
    glEnableClientState(GL_VERTEX_ARRAY);
        glBindBuffer(GL_ARRAY_BUFFER, _vbo_data);
            ...
            GLuint offset= 0;
            //a sorrend attol fugg, hogy a puffert hogyan toltottem fel, ezert van hogy egyszer a soronkent ragadjuk ki, aztan oszlopra
            for(GLuint i = 0; i< _data.GetRowCount(); i++, offset += _data.GetColumnCount())
            {
                glDrawArrays(render_mode, offset, _data.GetColumnCount());//render_mode- milyen tipust, 0-jelenti az offsettet, hogy honnan
                                                              //akarjuk kirajzolni
            }
            for(GLuint j = 0; j< _data.GetColumnCount(); j++, offset += _data.GetRowCount())
            {
                glDrawArrays(render_mode, offset, _data.GetRowCount());//render_mode- milyen tipust, 0-jelenti az offsettet, hogy honnan
                                                              //akarjuk kirajzolni
            }
...
GLboolean TensorProductSurface3::UpdateVertexBufferObjectsOfData(GLenum usage_flag)
{
    ...
    //vegigmegy a kontrollpontokon es a j megmondta hogy melyiknel vagyok(atirtuk c-re), j-az oszlopokon megy vegig
                                                      ...
                                                      GLenum usage_flag) const
{
    ...
    {
        (*result)[i]=new GenericCurve3(max_order_of_derivatives, div_point_count,usage_flag);
        if(!(*result)[i])
        {
            //innen clean-up
        ...
        for(GLuint j=0; j<div_point_count; j++)
        {
            ...
            PartialDerivatives pd;
            if(!CalculatePartialDerivatives(max_order_of_derivatives, u, v, pd))
            {
                for(GLuint k=0; k<=i; k++)
                {
                    delete (*result)[k];
            ...
            {
                //(*result) ez a sormatrixra
                (*(*result)[i])(r, j)=pd(r,0);//r-hanyad rendu , ! v irany eseten pd(r,r)
            }
        }
    }
    return result;
                                                      ...
                                                      GLenum usage_flag) const
{
    ...
    for(GLuint i=0; i<iso_line_count; i++)
    {
        ...
        for(GLuint j=0; j<div_point_count; j++)
        {
            GLdouble u=min(_u_min + j*u_step, _u_max);
            PartialDerivatives pd;
            if(!CalculatePartialDerivatives(max_order_of_derivatives, v , u, pd))
            {
                for(GLuint k=0; k<=i; k++)
                {
                    delete (*result)[k];
            ...
            {
                //(*result) ez a sormatrixra
                (*(*result)[i])(r, j)=pd(r,r);//r-hanyad rendu , ! v irany eseten pd(r,r)
            }
        }
    }
    return result;
